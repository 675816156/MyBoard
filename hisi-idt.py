...
class bootdownload(object):
	...
	startframe = {
		'hikey960':[0xFE,0x00,0xFF,0x01,0x00,0x00,0x00,0x04,0x00,0x00,0x02,0x01]
	}
	...
	def download(self, filename1, filename2, filename3):
		f=open(filename1,"rb")
		data = f.read()
		f.close()

		print('Sending', filename1, '...')
		self.senddata(data,self.firstbootaddress[self.chip])
		print('Done\n')
		...
		self.senddata(data,self.secondbootaddress[self.chip])
		print('Done\n')
		...
		self.senddata(data,self.thirdbootaddress[self.chip])
		print('Done\n')


def burnboot(chiptype, serialport, filename1, filename2, filename3):
	downloader = bootdownload(chiptype, serialport)
	downloader.download(filename1, filename2, filename3)

def main(argv):
	img1 = './sec_usb_xloader.img'
	img2 = './sec_uce_boot.img'
	img3 = './fastboot.img'
	dev  = 'COM3'
	try:
		opts, args = getopt.getopt(argv,"hd:")
	except getopt.GetoptError:
		print('hisi-idt.py -d <COMn>')
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print('hisi-idt.py -d <COMn>')
	...
	burnboot('hikey960', dev, img1, img2, img3)
